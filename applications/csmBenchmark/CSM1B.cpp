...
/*=====================================================================*/
void CSMTestCases::createCSM1BBenchmark()
{
    string pathname = "c:/temp/outputFSW/";
    TmcFileOutputASCII *logfile = createFoldersAndGetLogfile(pathname);

    logfile->writeLine("CSM-1B--Benchmark for FE-Code !!!");

    double dTstructure = 0.0;

    cout << "Structure timestep:" << dTstructure << endl;
    logfile->writeString("Structure timestep:");
    logfile->writeDouble(dTstructure);
    logfile->writeLine();
    ...
    double qLast = -40.0; // 2.*m; //40.;//-0.5;// -1000000.;
    ...
    cout << beam->getKMatrix()->toString() << endl;
    cout << beam->getMMatrix()->toString() << endl;
    std::vector<double *> result = solver.getCalculatedStartSolution(&lastvector);
    std::cout << "CSM 1B - displacements:" << std::endl;
