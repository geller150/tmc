...
public:
    ...
    /*==========================================================*/
    TmcTiming(const std::string &name)
    {
        this->duration = 0.0;
        this->deltaT = 0.0;
        this->startTime = 0;
...
/*=========================================================================*/
//  TmcTimer - Time Measuring
//
//
//
#include "TmcSystem.h" //for definitons of system/OS type
...
{
public:
    TmcTimer(const bool &storeLapTimes = false)
        : name("unamed"), isMeasuring(false), storeLapTimes(storeLapTimes), startTime(0.0), totalTime(0.0), lapTime(0.0)
    {
    }
    /*==========================================================*/
    TmcTimer(const std::string &name, const bool &storeLapTimes = false)
        : name(name), isMeasuring(false), storeLapTimes(storeLapTimes), startTime(0.0), totalTime(0.0), lapTime(0.0)
    {
    }
    /*==========================================================*/
    virtual ~TmcTimer() {}
    ...
    /*==========================================================*/
    friend inline std::ostream &operator<<(std::ostream &os, const TmcTimer &timer)
    {
        os << "TmcTimer[totalTime=" << timer.totalTime << "sec, lapTimes(";
...
/*=========================================================================*/
//  TmcProgressTimer - Time Measuring
//
//
//
// TmcProressTimer measures the time from initialization until destroying
// and output the elapsed time on "os" in [s]
// example:
// {
//    TmcProgressTimer timer;
//    TmcSystem::sleepS(10);
// } //--> 10s
class TmcProgressTimer : public TmcTimer
{
private:
    TmcProgressTimer(const TmcProgressTimer &rhs);

public:
    explicit TmcProgressTimer(std::ostream &os = std::cout)
        : TmcTimer(), os(os)
    {
        this->start();
    }
